...
object Downloader {
     ...
     * @return Result<DownloadResult> 包含成功下载的内容及响应头或失败的异常
     */
    suspend fun download(url: String): Result<DownloadResult> {
        return withContext(Dispatchers.IO) {
            runCatching {
                ...
                client.newCall(request).execute().use { response ->
                    ...
                    val interval = response.header("profile-update-interval")
                    AppLogger.d("DI_DOWNLOADER: 下载成功, 内容长度: ${body.length}, userinfo: $userinfo")
                    DownloadResult(body, userinfo, interval)
                }
            }
        }
    }
